...
## Probabilistic Scenario-Based Design: Generating insight from probability-based theoretical user models
...
library(plyr)
source("./input.data.R")
setwd("/users/scottsfarley/documents/geog970_eval")
```
...
```{r, echo=F}
student.model <- read.csv("./../../models/student.model.csv")
research.model <- read.csv("./../../models/research.model.csv")
...
```{r, echo=F}
N = 1000
student.dat <- generateTheoreticalData(N, "Student", "./../../models/student.model.csv")
research.dat <- generateTheoreticalData(N, "Research", "./../../models/research.model.csv")

kable(head(student.dat[1:5]))
```
...
ggplot(mem, aes(x = C1, y=C2, color=C1, shape=class)) + geom_point(size=3) + ggtitle("Percent cluster membership") + xlab("Probabiity of Research ->") + ylab("Probabiity of Student ->") + scale_color_continuous("Modeled membership\npercentage of research class") + scale_shape("Actual class")
